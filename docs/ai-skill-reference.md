...
| Tier | Command | Scope | What it does |
|---|---|---|---|
| Global | `bun bin/register-mcp.ts` | Once per machine | Registers MCP server in IDE client global configs |
| Per-project | `locksmith <path> --install-mcp` | Once per project | Copies `ai-skill.md` to project rule directories |

Both are required for IDE-integrated clients (Cursor, Windsurf, Cline).

### Client support matrix

| Client | Global MCP config | Per-project rules | Notes |
|---|---|---|---|
| Claude Desktop + Claude Code | `claude_desktop_config.json` | `~/.claude/CLAUDE.md` (appended) | Primary target |
| Cursor | `~/.cursor/mcp.json` | `.cursor/rules/locksmith.md` | Both tiers needed |
| Windsurf | `~/.codeium/windsurf/mcp_config.json` | `.windsurf/rules/locksmith.md` | Both tiers needed |
| Cline | VS Code extension settings | `.clinerules/locksmith.md` | Both tiers needed |
| Kiro (Amazon) | `~/.kiro/settings/mcp.json` | `~/.kiro/steering/locksmith.md` | Global only |
| Trae (ByteDance) | — | `.trae/mcp.json` + `.trae/rules/locksmith.md` | Per-project only |
| GitHub Copilot | — | `.github/copilot-instructions.md` | Rules only, no MCP |
| Google Antigravity | — | `AGENTS.md` | Rules only, MCP experimental |
